...
public class RecyclerViewAdapter extends RecyclerView.Adapter<RecyclerViewAdapter.MyViewHolder> {
	...
	class MyViewHolder extends RecyclerView.ViewHolder implements View.OnClickListener {
		...
		public void onClick(View v) {
			...
			default:
				if (!apps) {
					ComponentName name = new ComponentName(appDataList0.get(getAdapterPosition()).getPackageName(),
					...
					Intent i = context.getPackageManager()
							.getLaunchIntentForPackage(appDataList0.get(getAdapterPosition()).getPackageName());
					if (i != null) {
						try {
							context.startActivity(i);
						} catch (Exception ex) {
	...
	public RecyclerViewAdapter(List<AppData> appDataList, Context context, boolean apps, String list,
	...
	public void setItems(List<AppData> newList) {
		...
		if (htExpanded == null) {
			...
			while (i.hasNext()) {
				...
				if (!htExpanded.containsKey(s.getPackageName()) && !TextUtils.equals(s.getCategory(), "app")) {
		...
		while (i.hasNext()) {
			AppData s = i.next(); // must be called before you can call i.remove()
			if (s.getCategory() == "app" || "sort".equals(list) || htExpanded.containsKey(s.getPackageName())) {
				appDataList0.add(s);
			}
		}
