			...
			public void run() {
				...
				while (!stop) {
					...
					for (ResolveInfo resolveInfo : resolveInfoList) {
						...
						if (cat != "activity" || htCollected.containsKey(resolveInfo.activityInfo.packageName)) {
							...
							try {
								...
								if (labelRes == 0)
									labelRes = resolveInfo.activityInfo.labelRes;
							...
							if (cat != "activity") {
								app.setName(resolveInfo.loadLabel(packageManager).toString());
								htCollected.put(resolveInfo.activityInfo.packageName,
					...
					if (category == "activity" && cat != "activity")
						cat = "activity";
					else {
						stop = true;
					}
