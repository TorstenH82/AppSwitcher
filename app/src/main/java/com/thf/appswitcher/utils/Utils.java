...
public class Utils {
  ...
  public static void enableDisableNaviMainActivity(
    ...
    String command =
        "pm " + (disable ? "disable" : "enable") + " " + context.getString(R.string.naviActivity);
  ...
  public static void enableDisableHideActivity(
      Context context, boolean disable, UtilCallbacks utilCallbacks) {
    String command =
        "pm " + (disable ? "disable" : "enable") + " " + context.getString(R.string.hideActivity);
    UtilCallbacks listener = utilCallbacks;
            ...
            new Runnable() {
              public void run() {
                ...
                try {
                  ...
                  throw new RuntimeException(
                      "Error enabling service: \"" + srv + "\" Exception: " + ex.getMessage());
                }
              }
            });
            ...
            new Runnable() {
              public void run() {
                try {
                  Intent i =
                      new Intent(
                          context.getString(R.string.automateFlowStartIntent), Uri.parse(uri));
  ...
  public static String getSetFullscreenFlag(Context context, String newValue)
    ...
    try {
      ...
      String chmodCommand =
          String.format(
              context.getString(R.string.commandChmod777), context.getFilesDir() + "/build.prop");
      ...
      if (modification) {
        ...
        */
        ...
        sudoForResult(context, copyCommand);
        chmodCommand =
            String.format(context.getString(R.string.commandChmod644), "/vendor/build.prop");
        sudoForResult(context, chmodCommand);
  ...
  public static void enableAutostart(Context context, boolean enable) throws SetAutostartException {
    try {
      ...
      sudoForResult(
          context, String.format("setprop %s \"%s\"", "sys.qb.startapp_onresume", newProp));
    } catch (SysPropException e) {
